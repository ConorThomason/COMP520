...
public class Checkpoint2 {
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		File classPath = new File(projDir + "/out/production/miniJava");
		...
		File fileToFind = new File(classPath + "/miniJava/Compiler.class");
		...
		for (File x : testDir.listFiles()) {
			if (x.getName().endsWith("out") || x.getName().startsWith("."))
				continue;
			ReturnInfo info = runTest(x, classPath);
	...
	private static ReturnInfo runTest(File x, File cp) throws IOException, InterruptedException {
